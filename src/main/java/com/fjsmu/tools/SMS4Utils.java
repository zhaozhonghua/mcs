...
public class SMS4Utils {
	...
	private   int SMS4Lt(int a)
	{
		int b = ByteSub(a);
		int c = L1(b);
		return c;
	}
	private   int SMS4F(int x0, int x1, int x2, int x3, int rk)
	{
    ...
    void SMS4Crypt(byte[] Input, byte[] Output, int[] rk) {
        int r,i, mid, x0, x1, x2, x3;
        int[] x = new int[36];
        int[] tmp = new int[4];
//        System.out.println(rk[0]+","+rk[1]+","+rk[2]+","+rk[3]);
        for (i = 0; i < 36; i++) {
		    if(i<4)
			{
            ...
            x[i] = (tmp[3] << 24) | (tmp[2] << 16) | (tmp[1] << 8) | tmp[0];
            //x[i] = tmp[3] << 24 | tmp[1] << 16 | tmp[2] << 8 | tmp[3];
		...
		//==============================
            while(i<32)
            {
                x[i+4] = SMS4F(x[i], x[i+1], 
                                   x[i+2], x[i+3], rk[i]);
                i++;
            }
        //System.out.println(x[35]+","+x[34]+","+x[33]+","+x[32]);
           
		//==============================
        // Reverse
        for (int j = 0; j < 16; j += 4) {
        	
            Output[j+3] = (byte) ((x[35 - j / 4] >>> 24) & 0xFF);
            Output[j + 2] = (byte) ((x[35 - j / 4] >>> 16) & 0xFF);
            Output[j + 1] = (byte) ((x[35 - j / 4] >>> 8 )& 0xFF);
            Output[j] = (byte) (x[35 - j / 4] & 0xFF);
        }
    ...
    public static byte[] encodeSMS4(String plaintext, byte[] key) {
        ...
        return SMS4Utils.encodeSMS4(plaintext.getBytes(), key);
    }
